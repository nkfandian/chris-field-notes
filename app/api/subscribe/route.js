...
export async function POST(request){
 try{
  const db=createAdminClient();if(!await enforceRateLimit(db,request,'subscribe',3,3600))return Response.json({error:'请求过于频繁，请稍后再试。'},{status:429})
  const body=await request.json(),email=cleanText(body.email,160).toLowerCase();if(body.website)return Response.json({ok:true});if(!validEmail(email))return Response.json({error:'请输入有效邮箱。'},{status:400})
  const {data:existing}=await db.from('subscribers').select('id,status,verified_at,confirmation_token').eq('email',email).maybeSingle()
  if(existing?.status==='active'&&existing.verified_at)return Response.json({ok:true,message:'这个邮箱已经完成订阅。'})
  ...
  return Response.json({ok:true,message:'确认邮件已经发送，请前往邮箱完成订阅。'},{status:202})
