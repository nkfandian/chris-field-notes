...
export default async function LogsPage({searchParams}){
  ...
  return <main className="logs-index"><StructuredData data={schema}/><nav><Link href="/">← 首页</Link><Link href="/" aria-label="CHRIS / FIELD NOTES 首页"><Logo compact/></Link></nav><header><h1>{domain==='all'?'日志':labels[domain].split('/ ')[1]}</h1><span>{posts.length} 篇</span></header><div className="logs-filters">{domains.map(item=><Link className={item===domain?'active':''} href={item==='all'?'/logs':`/logs?domain=${item}`} key={item}>{item==='all'?'全部':labels[item].split('/ ')[1]}</Link>)}</div><section>{posts.map((post,index)=><Link className="log-index-entry" href={`/logs/${encodeURIComponent(post.slug)}`} key={post.id}><span>{String(index+1).padStart(2,'0')}</span><div><small>{labels[post.domain]?.split('/ ')[1]||post.domain}</small><h2>{post.title}</h2><p>{post.excerpt}</p></div><time>{post.published_at?.slice(0,10)}</time></Link>)}</section></main>
}
