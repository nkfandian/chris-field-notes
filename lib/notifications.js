...
export async function deliverPending({postIds}={}){
 ...
 for(const post of posts||[]){
  ...
  for(const subscriber of subscribers||[]){
   ...
   const response=await fetch('https://api.resend.com/emails',{method:'POST',headers:{authorization:`Bearer ${process.env.RESEND_API_KEY}`,'content-type':'application/json'},body:JSON.stringify({from:process.env.NOTIFICATION_FROM,to:[subscriber.email],subject:`新日志｜${post.title}`,text:`${post.excerpt||emailPreview(post.body)}\n\nhttps://www.chrisreading.ink/logs/${encodeURIComponent(post.slug)}`,html:emailHtml(post,subscriber.manage_token)})})
