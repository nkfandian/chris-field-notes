...
export async function deliverPending({postIds}={}){
 ...
 const [{data:posts,error:postError},{data:subscribers,error:subscriberError}]=await Promise.all([postQuery,db.from('subscribers').select('id,email').eq('status','active').order('created_at')])
 ...
 for(const post of posts||[]){
  ...
  for(const subscriber of subscribers||[]){
   ...
   const response=await fetch('https://api.resend.com/emails',{method:'POST',headers:{authorization:`Bearer ${process.env.RESEND_API_KEY}`,'content-type':'application/json'},body:JSON.stringify({from:process.env.NOTIFICATION_FROM,to:[subscriber.email],subject:`新日志｜${post.title}`,html:emailHtml(post)})})
